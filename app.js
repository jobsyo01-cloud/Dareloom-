...
async function renderSearch(query,page=1){
  ...
  let filtered = data.filter(d=>d.Title?.toLowerCase().includes(lowerQuery));
  const {pageItems,total} = paginate(filtered,page,PAGE_SIZE);
  qs('#list').innerHTML = pageItems.length>0 ? pageItems.map(movieCardHtml).join('') : `<p class="not-found" style="text-align:center;padding:40px;">No results found for "${query}".</p>`;
...
async function renderCategory(cat,page=1){
  ...
  let filtered = data.filter(d=>d.Category?.toLowerCase().includes(lowerCat)||d.Genre?.toLowerCase().includes(lowerCat));
  const {pageItems,total} = paginate(filtered,page,PAGE_SIZE);
...
async function router(){
  ...
  if(parts[0]==='category'){
    ...
    const page = parts[2]==='page'?Number(parts[3]||1):1;
